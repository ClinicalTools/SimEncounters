...
public class MoveableObjectCursorScript : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler {
	...
	private CECursor cursor;					//Reference to the Cursor object
	...
	private HistoryFieldManagerScript hm;		//Pointer to the manager script
	...
	public void UpdateEntryList() {
		if (entries != null) {
			...
			for(int i = 0; i < container.parent.childCount; i++) {
				//foreach (Transform entry in container.parent.GetComponentsInChildren<Transform>()) {
				//if (entry.parent == container.parent && entry.name != container.name) {
				if (container.parent.GetChild(i).name != container.name) {
					entries.Add(container.parent.GetChild(i));
				}
				//}
			}
		}
